...
cd ~/pi-gen
# Copy our overlay in as stage-acursed
cp -R /path/to/aeon-cursed-kvm/image-builder/stage-acursed .
echo IMG_NAME=aeon-cursed-kvm > config
...
Output: `deploy/<date>-aeon-cursed-kvm.img.xz`. Flash with `rpi-imager`,
`dd`, or BalenaEtcher.
...
| `01-base/01-copy-binaries.sh` | Drop the three Rust daemons (built ahead of time for `aarch64-unknown-linux-gnu`) and the SvelteKit static build into `/usr/local/bin` and `/usr/share/acursed/web` |
| `02-services/00-run.sh` | Install systemd units, enable them |
| `02-services/01-network.sh` | NetworkManager + Stargate-or-AP-fallback watchdog (mirrors the `eye` setup) |
