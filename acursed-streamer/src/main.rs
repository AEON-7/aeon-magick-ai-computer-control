...
//!
//! Exposes its own HTTP API on a unix socket (`/run/acursed/streamer.sock`)
//! that `acursed-supervisor` proxies to the web UI.
...
struct Cli {
    /// Path to TOML config (defaults to /etc/acursed/streamer.toml then $CWD/streamer.toml)
    #[arg(short, long)]
